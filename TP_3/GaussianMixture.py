...
Test de l'utilisation de la GaussianMixture sur le jeu d'entrainement que l'on a,
...
class GaussianMixture:
    ...
    def compute_M_step(self,data):
        ...
        for k in range(self.k):
            ...
            self.Sigma_list[k] = sum([self.q_e_step[i,k]*np.dot((data[i,:] -self.mu_list[k]).transpose(),(data[i,:] -self.mu_list[k])) for i in range(data.shape[0])])
            self.Sigma_list[k]= self.Sigma_list[k]/np.sum(self.q_e_step[:,k])
